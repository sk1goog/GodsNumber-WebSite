...
// Liest data-colors und data-algo aus und initialisiert mehrere Simulatoren

document.addEventListener("DOMContentLoaded", () => {
  // Für jede Simulator-Instanz
  document.querySelectorAll(".simulator-wrapper").forEach(wrapper => {
    ...
    function applyColorString(str){
      ...
      const sorted = {
        U: cubies.filter(c=>isNear(c.position.y,1.05)).sort((a,b)=>a.position.z-b.position.z||a.position.x-b.position.x),
        R: cubies.filter(c=>isNear(c.position.x,1.05)).sort((a,b)=>b.position.y-a.position.y||b.position.z-a.position.z),
        F: cubies.filter(c=>isNear(c.position.z,1.05)).sort((a,b)=>b.position.y-a.position.y||a.position.x-b.position.x),
        D: cubies.filter(c=>isNear(c.position.y,-1.05)).sort((a,b)=>b.position.z-a.position.z||a.position.x-b.position.x),
        L: cubies.filter(c=>isNear(c.position.x,-1.05)).sort((a,b)=>b.position.y-a.position.y||a.position.z-b.position.z),
        B: cubies.filter(c=>isNear(c.position.z,-1.05)).sort((a,b)=>b.position.y-a.position.y||b.position.x-a.position.x)
      };
    ...
    const moveMap = {
      ...
      M:{axis:new THREE.Vector3(1,0,0),dir: 1,slice:c=>Math.abs(c.position.x)<.01},
      E:{axis:new THREE.Vector3(0,1,0),dir: 1,slice:c=>Math.abs(c.position.y)<.01},
      S:{axis:new THREE.Vector3(0,0,1),dir:-1,slice:c=>Math.abs(c.position.z)<.01},
    ...
    function parseAlgorithm(str){
      return str.trim().split(/\s+/).map(tok=>({
        face:tok[0], count:tok.endsWith("2")?2:1, prime:tok.endsWith("'")
      }));
    }
    ...
    function animateMove({face,count,prime},done){
      ...
      const dir = m.dir*(prime?-1:1), total=Math.PI/2*count; let rotated=0;
      ...
      (function step(){
        ...
        tmp.rotateOnAxis(m.axis, dir*delta);
        rotated+=delta;
        if(rotated<total) requestAnimationFrame(step);
        else{sliceCubies.forEach(c=>cubeGroup.attach(c)); scene.remove(tmp); done();}
      })();
    }
    let animating=false;
    function runSequence(seq){ if(!seq.length){animating=false;return;} animating=true; animateMove(seq.shift(),()=>runSequence(seq)); }
