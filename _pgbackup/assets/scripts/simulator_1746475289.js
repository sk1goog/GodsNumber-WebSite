...
(function() {
  ...
  // Aufbau des Würfels mit korrekt gruppierten Materialien (eine Seite = ein Material)
...
for (let x = -1; x <= 1; x++) {
  for (let y = -1; y <= 1; y++) {
    for (let z = -1; z <= 1; z++) {
      ...
      // Materialien: [R, L, U, D, F, B]
      const mats = [
        new THREE.MeshBasicMaterial({ color: x === 1 ? colors.R : 0x000000 }), // R
        new THREE.MeshBasicMaterial({ color: x === -1 ? colors.L : 0x000000 }), // L
        new THREE.MeshBasicMaterial({ color: y === 1 ? colors.U : 0x000000 }), // U
        new THREE.MeshBasicMaterial({ color: y === -1 ? colors.D : 0x000000 }), // D
        new THREE.MeshBasicMaterial({ color: z === 1 ? colors.F : 0x000000 }), // F
        new THREE.MeshBasicMaterial({ color: z === -1 ? colors.B : 0x000000 })  // B
      ];

      // Geometrie mit Gruppenzuweisung
      const geom = new THREE.BoxGeometry(size, size, size);
      geom.clearGroups(); // vorherige Gruppen entfernen
      for (let i = 0; i < 6; i++) {
        geom.addGroup(i * 6, 6, i); // 6 Indices = 1 Seite, i = Materialindex
      }
  ...
  cubies.forEach(cubie => {
    ...
    if (pos.x === 1) {
      ...
      console.log("Material vorher (R):", mats[0].color.getHexString());
      mats[0].color.setHex(colorMap[farbe]);
      mats[0].needsUpdate = true;
      console.log("Material nachher (R):", mats[0].color.getHexString());
    }

    if (pos.x === -1) {
      const farbe = facelets.L.shift();
      console.log("L-Face", pos, "→", farbe);
      console.log("Material vorher (L):", mats[1].color.getHexString());
      mats[1].color.setHex(colorMap[farbe]);
      mats[1].needsUpdate = true;
      console.log("Material nachher (L):", mats[1].color.getHexString());
    }

    if (pos.y === 1) {
      const farbe = facelets.U.shift();
      console.log("U-Face", pos, "→", farbe);
      console.log("Material vorher (U):", mats[2].color.getHexString());
      mats[2].color.setHex(colorMap[farbe]);
      mats[2].needsUpdate = true;
      console.log("Material nachher (U):", mats[2].color.getHexString());
    }

    if (pos.y === -1) {
      const farbe = facelets.D.shift();
      console.log("D-Face", pos, "→", farbe);
      console.log("Material vorher (D):", mats[3].color.getHexString());
      mats[3].color.setHex(colorMap[farbe]);
      mats[3].needsUpdate = true;
      console.log("Material nachher (D):", mats[3].color.getHexString());
    }

    if (pos.z === 1) {
      const farbe = facelets.F.shift();
      console.log("F-Face", pos, "→", farbe);
      console.log("Material vorher (F):", mats[4].color.getHexString());
      mats[4].color.setHex(colorMap[farbe]);
      mats[4].needsUpdate = true;
      console.log("Material nachher (F):", mats[4].color.getHexString());
    }

    if (pos.z === -1) {
      const farbe = facelets.B.shift();
      console.log("B-Face", pos, "→", farbe);
      console.log("Material vorher (B):", mats[5].color.getHexString());
      mats[5].color.setHex(colorMap[farbe]);
      mats[5].needsUpdate = true;
      console.log("Material nachher (B):", mats[5].color.getHexString());
    }
  });
