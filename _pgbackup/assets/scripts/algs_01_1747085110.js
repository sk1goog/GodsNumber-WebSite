...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".simulator-wrapper").forEach(wrapper => {
    ...
    const initialTransforms = cubies.map(c => ({ pos: c.position.clone(), quat: c.quaternion.clone() }));
    ...
    function parseAlgorithm(str) {
      return str.trim().split(/\s+/).map(tok => ({ face:tok[0], count:tok.endsWith("2")?2:1, prime:tok.endsWith("'") }));
    }
    ...
    function highlightNext() {
      const spans = algoContainer.querySelectorAll(".move");
      spans.forEach(s => s.classList.remove("next"));
      if (nextIndex < spans.length) {
        spans[nextIndex].classList.add("next");
      }
    }

    // Animations-Engine
    function animateMove(move, done) {
      const m = moveMap[move.face]; if (!m) { done(); return; }
      const dir = m.dir * (move.prime?-1:1), total=Math.PI/2*move.count;
      let rotated=0;
      const sliceCubies = m.whole?cubies.slice():cubies.filter(m.slice);
      const tmp=new THREE.Group(); scene.add(tmp); sliceCubies.forEach(c=>tmp.attach(c));
      (function step(){
    ...
    function runSequence(seq, onDone) {
      if(!seq.length) { animating=false; onDone(); return;} animating=true;
    ...
    startBtn.addEventListener("click",()=>{
      if(animating) return;
      startBtn.disabled=true; stepBtn.disabled=true;
    ...
    resetBtn.addEventListener("click",()=>{
      animating=false; startBtn.disabled=false;
      nextIndex=0;
      cubies.forEach((c,i)=>{ c.position.copy(initialTransforms[i].pos); c.quaternion.copy(initialTransforms[i].quat); });
      applyColorString(colorStr);
      highlightNext();
    });
